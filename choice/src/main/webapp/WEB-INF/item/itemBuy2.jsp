            ...
            <div class="col-md-12 col-sm-12 mt-2 mb-5">
               <div class="table table-responsive">
                ...
                <c:forEach items="${cartlist}" var="i" >
                    <c:set var="total" value="${total + i.cart_item_sellprice }" />
                     <tr>
                 <td nowrap style="padding: 1px;">&nbsp <img
                           src="../resources/itemUpload/${i.item_thumbnail}" alt="${i.item_thumbnail}"
                           height="50" width="70">&nbsp${i.item_name }
                        </td>
                        <td nowrap>&nbsp${i.cart_item_sellprice}</td>
                        <td nowrap>&nbsp${i.cart_item_count }</td>
                        <td nowrap >${i.cart_totalsum }</td>
...
<%--                        <tr> <td nowrap style="padding: 1px;">&nbsp <img
                           src="../resources/images/${i.item_thumbnail}" alt="${i.item_thumbnail}"
                           height="50" width="70">&nbsp${i.item_name }
                        </td>
...
.row-bg-grey {
   background: #d1d3d4;
            ...
            <div class="col-md-12 col-sm-12 mt-2 mb-5  ">
                  ...
                  <div class="table table-responsive">
                     ...
                     <input type="hidden" id="item_idx" name="item_idx" value="${i.item_idx}"> --%>
            ...
            <div class="col-md-12 col-sm-12 mt-2 mb-5  ">
               ...
               <div class="table table-responsive">
           <form  action="orderList_ok2" name="push1" method="get">
                     <table class="table">

                        <tr>
                           <td class="row-bg-grey">수령인</td>
                           <td class="bg-white"><input type="text"
                              placeholder="수령인을 입력해주세요" name="or_receiver"></td>
                        </tr>
                        <tr>
                           <td class="row-bg-grey" nowrap>휴대전화</td>
                           <td class="bg-white"><input type="text" value="휴대전화" name="or_phone"></td>
                        </tr>
                        <tr>
                           ...
                           <td class="bg-white"><input type="button"
                              ...
                              name="or_address" style="width: 450px;" readonly> <br> <input
                              type="text" id="sample6_detailAddress" placeholder="상세주소" name="or_address2">
                           </td>
                        </tr>
                        <tr>
                           <td class="row-bg-grey">이메일</td>
                           <td class="bg-white"><input type="email" placeholder="이메일" name="or_email"></td>
                        </tr>
                     </table>
                     <input type="hidden" id="hBuySum" name="hBuySum"> 
                     <input type="hidden" id="hCoupon" name="hCoupon"> 
                     <input type="hidden" id="hTotalSum" name="hTotalSum" value="">
               <!-- itemDetail에서 get방식으로 받아온 count와 item_idx를 hidden으로 보냄. -->
               <input type="hidden" id="hCouponIdx" name="hCouponIdx" value="0" >
                 <c:forEach items="${ cartlist}" var="i">
                 <input type="hidden" id="hCount" name="hCount" value="${i.cart_item_count }">
                 <input type="hidden" id="item_idx" name="item_idx${i.cr_item_idx}" value="${i.cr_item_idx}">
                 <input type="hidden" name="cart_idx${i.cart_idx }" value="${i.cart_idx}">
                 </c:forEach>
            ...
            <div class="col-md-12 col-sm-12 mt-2 mb-1  ">
                  ...
                  <div class="table table-responsive"
                             ...
                             if($("input[id=price]").is(":checked")){
                                ...
                                if( $('input:checkbox[id=price]:checked').length>1 ) {
                                   ...
                                   var totalSum = document.getElementById('totalSum');
                                     totalSum.innerHTML = (delivery + (Number(total)) - price);
                                     
                                   //hidden form으로 보낼것들
                                     //결제금액
                                     var hBuySum = document.getElementById('hBuySum');
                                     hBuySum.value = Number(total);
                                     //쿠폰
                     ...
                     //주문합계금액
                     var buySum = document.getElementById('buySum').innerHTML = total +"원"
                     
                     //최종결제금액
                     var totalSum = document.getElementById('totalSum');
                            totalSum.innerHTML = (delivery + (Number(total)));
            ...
            <div class="col-md12 col-sm-12 mb-5">
               <div style="margin: 0 auto; width: 50%;">
                  <button class="btn btn-primary mr-3 " style="width: 40%;">취소</button>

                  <!-- form안에서가 아닌 바깥에서 결제하기버튼을 눌렀을때 submit하기 위해 form에 id값을 주고
                  결제하기버튼에서 form="push1"로 form의id를 입력하고 type="submit"를 해주었다.-->
                  <button type="button" onClick="javascript:TwoSubmit()" class="btn btn-warning ml-3 text-white" style="width: 40%;">결제하기</button>
                     <script>function TwoSubmit()
                     {
                           document.push1.submit();
   ...
   <script>
      ...
      <!-- 뒤로가기를 눌렀을때 이 페이지로 오는걸 방지. 
         itemEnd에서 뒤로가기눌러 이 페이지로 오고나서 다시 결제하기를 누르면 데이터에 값이 입력되기때문에 방지해야한다. -->
