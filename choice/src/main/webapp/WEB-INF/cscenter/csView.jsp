...
<body>
   ...
   <div class="header">
      ...
      <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-warning">
         ...
         <a class="navbar-brand" href="http://localhost:8081/csList">고객센터페이지-고객상담센터</a>
         <!-- 로그인 프로필 logprofile -->
         <%
      ...
      <div class="container">
         <div class="row">
            ...
            <div class="col-md-10 col-sm-12" style="height: 200px;">
               ...
               <p>제목 : ${ dto.cs_title }</p>
               <hr>
               <p>작성자 : ${ dto.cs_writer }</p>
               <hr>
               <p>내용</p>
               <div class="shadow-sm p-3 mb-5 bg-white rounded">${ dto.cs_content }</div>

               <!--  글수정클릭시CscenterModify로이동 -->
               <input type="submit" class="btn btn-success pull-right" value="글수정"
                  onClick="CscenterModify()" />

               <!-- 글삭제  클릭시CscenterDelete로이동 -->
               <input type="submit" class="btn btn-success pull-right" value="글삭제"
                  onClick="CscenterDelete()" />
            </div>
      ...
      <div class="container">
         <div class="row">
            ...
            <nav aria-label="Page navigation example" style="margin: auto;">
               <ul class="pagination">
                  <li class="page-item"><a class="page-link" href="csList">뒤로가기</a></li>
               </ul>
            </nav>
...
//글삭제
   /* confirm창 띄우기 */
   function CscenterDelete() { 
   
   <%String user_id = (String) session.getAttribute("login");%>
   
   /* 작성자ID와 로그인ID일치시 confirm창 띄움.  */
   /* ""있어야함. equals불가. */
    if("${ dto.cs_writer }" == "<%=user_id%>") {
          var Cscenterleave = confirm("정말로 삭제하시겠습니까?");
            
            //확인 클릭시 이동
            if(Cscenterleave == true) {
               location.href="Cscenterdelete_ok?cs_idx=${dto.cs_idx}"
     ...
     //글수정
     /* 작성자 id와 로그인id일치시 글수정 폼으로 들어갈수있음. */
     function CscenterModify() { 

    if("${ dto.cs_writer }"=="<%=user_id%>"){
      
       
            //확인 클릭시 이동
            location.href = "csmodifyForm?cs_idx=${dto.cs_idx }"
            //취소 클릭시 아무것도 없음
